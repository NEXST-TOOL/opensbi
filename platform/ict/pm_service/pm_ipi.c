...
volatile struct apu_ipc {
  uint32_t req; // write 1 by RV; write 0 by ARM
  uint32_t msg[8];
  uint32_t resp[8];
} *apu_ipc_base = (void*)0x1ff00000;
					     ...
					     uint32_t is_blocking)
{
	...
	for (unsigned long i = 0; i < PAYLOAD_ARG_CNT; i++) {
...
{
	unsigned int offset = 0;
	uintptr_t buffer_base = (uintptr_t)apu_ipc_base->msg;
#if ZYNQMP_IPI_CRC_CHECK
	payload[PAYLOAD_CRC_POS] = calculate_crc(payload, IPI_W0_TO_W6_SIZE);
#endif
					   ...
					   unsigned int *value, unsigned long count)
{
	...
	unsigned int response_payload[PAYLOAD_ARG_CNT];
#endif
	uintptr_t buffer_base = (uintptr_t)apu_ipc_base->resp;
 ...
 * @return	Returns status, either success or error+reason
 */
void pm_ipi_buff_read_callb(unsigned int *value, unsigned long count)
{
	unsigned long i;
#if ZYNQMP_IPI_CRC_CHECK
	unsigned long j;
